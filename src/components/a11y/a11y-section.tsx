            ...
            return (
              <div key={i} className="rounded-2xl p-4 space-y-2 min-h-[100px] flex flex-col justify-between border border-default"
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className={`font-semibold ${h.aa ? "text-green-300" : "text-red-300"}`}>{h.rating} {h.ratio.toFixed(1)}:1</span>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {sorted.slice(0, 12).map((p, i) => (
            <div key={i} className="flex items-center gap-2 text-sm py-1.5 border-b border-default">
              ...
              <span className={`font-mono text-xs font-semibold ml-auto ${p.aa ? "text-green-500" : "text-red-500"}`}>{p.ratio.toFixed(2)}:1</span>
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${p.aaa ? "bg-green-500/10 text-green-500" : p.aa ? "bg-amber-500/10 text-amber-500" : "bg-red-500/10 text-red-500"}`}>{p.aaa ? "AAA" : p.aa ? "AA" : "Fail"}</span>
            </div>
          ))}
        </div>
      </div>
              ...
              {TYPOGRAPHY_SIZES.map((t) => {
                ...
                return (
                  <div key={t.label} className="flex items-center justify-between gap-2">
                    ...
                    <span className={`text-[10px] font-bold ${aaa ? "text-green-500" : aa ? "text-amber-500" : "text-red-500"}`}>
                      {aaa ? "AAA" : aa ? "AA" : "Fail"}
                    </span>
                  </div>
                );
      ...
      <div className="rounded-2xl border border-default p-4 space-y-2">
        ...
        {[
          ...
          <div key={item.label} className="flex items-center gap-2 text-sm">
            <span className={item.pass ? "text-green-500" : "text-red-500"}>{item.pass ? "✓" : "✗"}</span>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {/* Light */}
        <div className="rounded-2xl border border-default p-4 space-y-3">
          <p className="text-sm font-bold text-page">Light Mode</p>
          <div className="flex gap-2">
            <div className="flex-1">
              <p className="text-[10px] text-muted">Bg</p>
              <input type="color" value={lightBg} onChange={(e) => setLightBg(e.target.value)} className="size-8 rounded cursor-pointer" />
            </div>
            <div className="flex-1">
              <p className="text-[10px] text-muted">Text</p>
              <input type="color" value={lightText} onChange={(e) => setLightText(e.target.value)} className="size-8 rounded cursor-pointer" />
            </div>
          </div>
          <div className="h-16 rounded-xl flex items-center justify-center" style={{ backgroundColor: lightBg, color: lightText }}>
            <span className="text-sm font-semibold">Sample text</span>
          </div>
          <div className="flex gap-2 text-xs">
            <span className={`font-bold ${lightRatio >= 7 ? "text-green-500" : lightRatio >= 4.5 ? "text-amber-500" : "text-red-500"}`}>{lightRatio.toFixed(2)}:1</span>
            <span className={`font-bold ${lightRatio >= 4.5 ? "text-green-500" : "text-red-500"}`}>{lightRatio >= 7 ? "AAA" : lightRatio >= 4.5 ? "AA" : "Fail"}</span>
          </div>
        </div>

        {/* Dark */}
        <div className="rounded-2xl border border-default p-4 space-y-3">
          <p className="text-sm font-bold text-page">Dark Mode</p>
          <div className="flex gap-2">
            <div className="flex-1">
              <p className="text-[10px] text-muted">Bg</p>
              <input type="color" value={darkBg} onChange={(e) => setDarkBg(e.target.value)} className="size-8 rounded cursor-pointer" />
            </div>
            <div className="flex-1">
              <p className="text-[10px] text-muted">Text</p>
              <input type="color" value={darkText} onChange={(e) => setDarkText(e.target.value)} className="size-8 rounded cursor-pointer" />
            </div>
          </div>
          <div className="h-16 rounded-xl flex items-center justify-center" style={{ backgroundColor: darkBg, color: darkText }}>
            <span className="text-sm font-semibold">Sample text</span>
          </div>
          <div className="flex gap-2 text-xs">
            <span className={`font-bold ${darkRatio >= 7 ? "text-green-500" : darkRatio >= 4.5 ? "text-amber-500" : "text-red-500"}`}>{darkRatio.toFixed(2)}:1</span>
            <span className={`font-bold ${darkRatio >= 4.5 ? "text-green-500" : "text-red-500"}`}>{darkRatio >= 7 ? "AAA" : darkRatio >= 4.5 ? "AA" : "Fail"}</span>
          </div>
        </div>
      </div>
    </div>
