          ...
          return (
            <div key={color.id} className="rounded-2xl p-4 bg-white/10 backdrop-blur space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-[10px] text-white/50">{nh}</span>
              </div>
              <p className="text-xs text-white/70"><span className="font-bold text-white/90">Psychology:</span> {info.psychology}</p>
              <p className="text-xs text-white/70"><span className="font-bold text-white/90">Meaning:</span> {info.meaning}</p>
              <div className="flex flex-wrap gap-1">
                {info.applications.map((a) => (
                  <span key={a} className="rounded-full bg-white/15 px-2 py-0.5 text-[10px] text-white/70">{a}</span>
                ))}
              </div>
            </div>
          );
...
function SimilarColorsSection({ hex }: { hex: string }) {
  ...
  return (
    <div className="rounded-2xl p-4 bg-white/10 backdrop-blur space-y-3">
      <h3 className="text-xs font-bold tracking-wider uppercase text-white/60">Similar Colors</h3>
      <div className="flex flex-wrap gap-2">
        {similar.map((s) => (
          <div key={s.hex} className="flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1">
            ...
            <span className="text-[10px] text-white/40">{s.distance.toFixed(0)}</span>
          </div>
        ))}
      </div>
    </div>
...
function PaletteAnalysisSection({ palette }: Props) {
  return (
    <div className="rounded-2xl p-4 bg-white/10 backdrop-blur space-y-3">
      <h3 className="text-xs font-bold tracking-wider uppercase text-white/60">Palette Analysis</h3>
...
function AnalysisCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-white/5 p-3">
      <p className="text-[10px] font-bold tracking-wider uppercase text-white/40">{label}</p>
